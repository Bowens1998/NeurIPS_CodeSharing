...
(async function () {
  ...
  function setMode(m) {
    ...
    if (m === 'human') {
      ...
      btn.auto.style.display = 'inline-block';
    }
  ...
  // --- Auto Run (single-session API-driven loop) ---
  async function callAPI(prompt) {
    ...
    if (fb >= 0) content = content.substring(fb);
  ...
  async function autoRunLoop() {
    ...
    if (!runLog) start(true);

    autoRunning = true;
    btn.auto.textContent = '⏹ Stop Auto Run';
    btn.auto.style.background = '#ef4444';

    try {
      while (autoRunning) {
        const targetTrials = Number(controls.trials.value) || cfg.trials_per_block;
        if (runLog.trials.length >= targetTrials) break;

        // Tick loop for current trial
        while (autoRunning && !done) {
          const prompt = llmPrompt.value;
          if (!prompt) break;

          const obj = await callAPI(prompt);
          if (!autoRunning) break;

          applyLLMResponse(obj);
          await new Promise(r => setTimeout(r, 200));

          // Auto-confirm finalization modal if open (ESI set by LLM)
          if (modalBg.style.display === 'flex') {
            confirmESI.click();
            break;
          }

          // Advance to next tick (may open modal at time limit)
          if (!done) {
            stepTick();
            await new Promise(r => setTimeout(r, 100));

            // Time limit reached — call API once more to get ESI, then confirm
            if (modalBg.style.display === 'flex') {
              try {
                const esiObj = await callAPI(llmPrompt.value);
                if (autoRunning) applyLLMResponse(esiObj);
                await new Promise(r => setTimeout(r, 200));
              } catch (e) { /* ignore; confirm with current UI state */ }
              confirmESI.click();
              break;
            }
          }
        ...
        // Start next trial if quota not yet met
        if (done && runLog.trials.length < targetTrials) {
      ...
      log('Auto run error: ' + e.message);
    }

    autoRunning = false;
    btn.auto.textContent = '🤖 Auto Run (API)';
    btn.auto.style.background = '#8b5cf6';
  }
