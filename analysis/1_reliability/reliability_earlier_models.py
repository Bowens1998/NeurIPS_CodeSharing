...
Output     : figures/reliability_earlier/
  Reliability_{task}_{model}_N90.png  — per-model 5-panel diagnostic
  Reliability_Summary_EarlierModels.png — cross-model P1/P2/IC summary table
...
from utils import EARLIER_KEYS, EARLIER_LABELS, TASKS

ROOT    = Path(__file__).parent.parent.parent
IC_DIR  = ROOT / 'data' / 'earlier_models' / 'reliability_N90'
FIG_DIR = ROOT / 'figures' / 'reliability_earlier'
FIG_DIR.mkdir(parents=True, exist_ok=True)

MODEL_LABELS = dict(zip(EARLIER_KEYS, EARLIER_LABELS))

CONDITION_COLORS = {1: '#3b82f6', 2: '#22c55e', 3: '#ef4444'}
RD_COLORS = {1: '#1e3a8a', 2: '#3b82f6', 3: '#94a3b8', 4: '#f59e0b', 5: '#dc2626'}


def load_model_task(model, task):
    fpath = IC_DIR / task / f'{model}_90.json'
    if not fpath.exists():
...
def plot_ic(ctx_arr, rd_arr, cond_arr, model, task, out_path):
    ...
    for i, c in enumerate(conditions):
        ...
        axC.text(min(s['ctx_mean'] + xerr_hi[i] + 1.5, 90), i,
                 f'{marker}  SD={s["ctx_sd"]:.1f}  RSD={s["ctx_rsd"]:.0f}%  band±5={s["ctx_band5"]:.0%}',
...
def plot_summary_table(all_results):
    fig, ax = plt.subplots(figsize=(14, 0.7 * len(EARLIER_KEYS) + 2.5))
    ax.axis('off')
    col_labels = ['Model',
                  'DNC\nP1', 'FIP\nP1', 'CTD\nP1',
                  'DNC\nP2', 'FIP\nP2', 'CTD\nP2',
    ...
    neu = '#f8fafc'
    alt = '#f1f5f9'
    for ri, m in enumerate(EARLIER_KEYS, start=1):
        for ci in range(len(col_labels)):
            cell = tbl[ri, ci]
            if ci == 0:
                cell.set_facecolor(neu if ri % 2 == 1 else alt)
    ...
    plt.tight_layout()
    out = FIG_DIR / 'Reliability_Summary_EarlierModels.png'
...
if __name__ == '__main__':
    print('=' * 60)
    print('Reliability Analysis — Earlier-Generation Models (N=90)')
    print('=' * 60)
