#!/usr/bin/env python3
"""
Step 3 — AUC Computation  (Frontier Models, Paper §3.3)
=========================================================
Integrates the OLR S-curve to produce a scalar AUC (Area Under Curve) per
model per task.  AUC = ∫₀¹ E[R_D | B_C] dB_C.

Reads from : utils.py  (shared extraction, K-means, OLR/AUC fitting)
Input data : data/frontier_models/main_N100/{DNC,CTD,FIP}/{task}_{model}_N100.json
Output     : figures/auc_frontier/
  AUC_Barchart_SortedBy{DNC,FIP,CTD}_FrontierModels.png
  AUC_Barchart_AllModels_FrontierModels.png
  AUC_Ranking_FrontierModels.png

Relation to other steps:
  ← Step 2 (olr_frontier_models.py) showed the B_C→R_D curves visually.
...
from utils import (
    FRONTIER_KEYS, FRONTIER_LABELS, TASKS, TASK_COLORS,
    load_frontier_task, discretize_frontier_task, fit_olr_auc,
)

ROOT = Path(__file__).parent.parent.parent
BASE = ROOT / 'data' / 'frontier_models' / 'main_N100'
OUT  = ROOT / 'figures' / 'auc_frontier'
...
print('=' * 60)
print('Step 3 — AUC / Beta Analysis  (Frontier Models, N=100)')
print('=' * 60)
...
for task in TASKS:
    ...
    for mkey in FRONTIER_KEYS:
        bc_arr, rd_arr = discretized[mkey]
...
rank_table = {}
for task in TASKS:
    pairs = [(m, results[task][m]['auc']) for m in FRONTIER_KEYS
...
def plot_auc_barchart(models_order, fname, sort_task=None):
    labels_order = [FRONTIER_LABELS[FRONTIER_KEYS.index(m)] for m in models_order]
...
def plot_ranking_chart(fname):
    ...
    for m in FRONTIER_KEYS:
        per_task  = [rank_table[t].get(m, len(FRONTIER_KEYS)) for t in TASKS]
        rank_data[m] = per_task + [np.mean(per_task)]

    model_order = sorted(FRONTIER_KEYS, key=lambda m: rank_data[m][-1])
    labels_ord  = [FRONTIER_LABELS[FRONTIER_KEYS.index(m)] for m in model_order]
    ...
    ax_bump.set_xticks(task_x)
    ax_bump.set_xticklabels(TASKS, fontsize=10.5, fontweight='bold')
    ...
    ax_bump.spines['top'].set_visible(False)
    ax_bump.spines['right'].set_visible(False)

    for model in model_order:
        ranks = [rank_data[model][j] for j in range(len(TASKS))]
        color = col_map[model]
        ax_bump.plot(task_x, ranks, '-o', color=color, linewidth=2.0,
                     markersize=9, zorder=4, solid_capstyle='round')
        lbl = FRONTIER_LABELS[FRONTIER_KEYS.index(model)]
        ax_bump.text(len(TASKS) - 0.55, ranks[-1], lbl,
...
for sort_task in TASKS:
    order = sorted(FRONTIER_KEYS, key=lambda m: results[sort_task][m]['auc'] or 0)
    plot_auc_barchart(order, f'AUC_Barchart_SortedBy{sort_task}_FrontierModels.png', sort_task)

mean_order = sorted(FRONTIER_KEYS, key=lambda m: np.mean(
    [results[t][m]['auc'] for t in TASKS if results[t][m]['auc'] is not None]))
plot_auc_barchart(mean_order, 'AUC_Barchart_AllModels_FrontierModels.png')
plot_ranking_chart('AUC_Ranking_FrontierModels.png')
